...
func TestCalculateOtpToken_NoPadding(t *testing.T) {
	secretNoPadding := "JBSWY3DPEHPK3PXP"

	result := calculateOtpToken(secretNoPadding, 1589112197)
	if result == "" {
		t.Error("calculateOtpToken without padding should work")
	}
	if len(result) != 6 {
		t.Errorf("OTP token should be 6 digits, got %s", result)
	}
}
...
func TestCalculateOtpToken_DifferentTimestamps(t *testing.T) {
	...
	result2 := calculateOtpToken(secret, 30)
	result3 := calculateOtpToken(secret, 60)
