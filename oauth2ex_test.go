...
func TestNewTokenRequest_SpecialCharacters(t *testing.T) {
	// Test that special characters in client ID/secret are NOT double-encoded.
	// SetBasicAuth already performs base64 encoding per RFC 7617.
	...
	// Basic auth should receive raw values (not URL-encoded)
	if username != clientID {
		t.Errorf("Basic auth username = %s; want %s", username, clientID)
	}
	if password != clientSecret {
...
func TestNewTokenRequest_NoDoubleEncoding(t *testing.T) {
	// Verify that credentials with special characters are NOT double-encoded.
	// This is a regression test for upstream Issue #10 where url.QueryEscape
	// was incorrectly applied before SetBasicAuth.
	...
	if username != clientID {
		t.Errorf("Basic auth username = %s; want %s (no URL encoding)", username, clientID)
	}
	if password != clientSecret {
		t.Errorf("Basic auth password = %s; want %s (no URL encoding)", password, clientSecret)
	}
}
